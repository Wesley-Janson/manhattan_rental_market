...
for (i in 1:floor(length(inventory_train) * 0.25)){
  ...
  ##### CHECK WITH WESLEY - START #######
  
  pred_exp_rent <- backout(var_forecast_exp$diff.BoxCox.rent_train..lambda...lambda_rent...differences...1.$mean, expand_window_rent, lambda_rent, 1, 1)
  pred_sl_rent <- backout(var_forecast_sl$diff.BoxCox.rent_train..lambda...lambda_rent...differences...1.$mean, rent_train, lambda_rent, 1, 1)
  
  ##### CHECK WITH WESLEY - END ##########

  exp_error <- exp_error + (mean(preds - pred_exp_rent)^2 / floor(length(inventory_train) * 0.25))
  sl_error <- sl_error + (mean(preds - pred_sl_rent)^2 / floor(length(inventory_train) * 0.25))

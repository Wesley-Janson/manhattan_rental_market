...
tsdisplay(BoxCox(inventory_train,lambda=lambda_inventory))
...
```{r}
# VAR Selection
VARselect(cbind(diff(BoxCox(rent_train, lambda = lambda_rent), differences = 1), diff(BoxCox(inventory_train,lambda=lambda_rent),differences=1,lag=1)), lag.max=12)$selection

inventory_var <- VAR(cbind(diff(BoxCox(rent_train, lambda = lambda_rent), differences = 1), diff(BoxCox(inventory_train,lambda=lambda_rent),differences=1,lag=1)), p=1, type='both', season=12)
...
# Plot of Inventory
plot(backout(var1_forecast$diff.BoxCox.inventory_train..lambda...lambda_rent...differences...1..$mean, inventory_train, lambda_rent, 1, 1), ylab="Inventory")
...
box_cox_rent_diff <- diff(box_cox_rent, differences = 1)

# Trying to undo this:
box_cox_rent2 <- diffinv(box_cox_rent_diff, differences = 1, xi=box_cox_rent[1])

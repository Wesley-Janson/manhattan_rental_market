...
# rent_arma_err_inv_model = function(training_data){
...
predict_and_forecast = function(model, 
                                covariate_series,
                                rent_series, 
                                i, 
                                last_training_point, 
  ...
  if (window_type=="expanding"){
    training_window <- ts(rent_series[1:last_training_point],frequency=12)
    covariate_train <- ts(covariate_series[1:last_training_point],frequency=12)
  } else if (window_type=="sliding"){
    training_window <- ts(rent_series[i:last_training_point],frequency=12)
    covariate_train <- ts(covariate_series[i:last_training_point],frequency=12)}
  last_test_point <- ifelse(
    last_training_point+n_forecast<=length(rent_series),
    last_training_point+n_forecast,
    length(rent_series))
  true_future <- rent_series[(last_training_point+1):(last_training_point+n_forecast)]
  model_trained <- model(training_window, covariate_train)
  model_aicc <- model_trained$aicc
  
  # forecase covariate
  covariate_arima <- auto.arima(covariate_series, 
                                lambda = 'auto')
  covariate_preds <- forecast(inventory_arima, h=12)$mean
  
  # predict
  forecast_error <- forecast(model_trained,
                             h=length(true_future),
                             xreg=covariate_preds)$mean-true_future 
  return(list(model_aicc,add_NAs_to_list(forecast_error)))
}

# cross-validation function
cross.validate = function(model, 
                          covariate_series, 
                          rent_series, 
                          n_window, 
                          n_forecast){
  ...
  # initialize
  iterations <- length(rent_series)-n_window
  ...
  for (i in 1:iterations){
    iteration_output <- predict_and_forecast(model,
                                             covariate_series,
                                             rent_series,
                                             i,
                                             i+n_window-1,
...
# plotting
generate.plots = function(output_arima){

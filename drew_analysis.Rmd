...
# Box-Cox transformed
tsdisplay(BoxCox(median_rent_manhattan,lambda='auto'))
tsdisplay(BoxCox(inventory_manhattan,lambda='auto'))
```
...
# median rent SARIMA
rent_sarima <- auto.arima(median_rent_manhattan,lambda='auto',trace=TRUE)
checkresiduals(rent_sarima)
```

Consider: might it be preferable to use ARIMA(2,1,1)(2,0,1)[12] with drift (4097.225) or ARIMA(3,1,1)(2,0,0)[12] with drift (4089.096) due to parsimony?

```{r}
occams_rent_sarima <- Arima(median_rent_manhattan,order=c(2,1,1),seasonal=c(2,0,1),lambda='auto',include.drift=TRUE)
checkresiduals(occams_rent_sarima)
```


```{r}
# for some reason the Box-Cox transform ruins the inventory SARIMA?
boxcox_inventory_sarima <- auto.arima(inventory_manhattan,lambda='auto')
checkresiduals(boxcox_inventory_sarima)
inventory_sarima <- auto.arima(inventory_manhattan,lambda=NULL,trace=TRUE)

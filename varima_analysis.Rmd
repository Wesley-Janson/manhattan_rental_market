...
kpss.test(diff(diff(BoxCox(data$inventory_train,lambda='auto'),differences=1,lag=12),differences=1,lag=1),null='Level')

# note: the kpss.test warning is because they did a shitty stupid implementation
# basically we'll never see a p-value above 0.1, but it is above that
```
...
# added:
tsdisplay(diff(diff(BoxCox(inventory_train, lambda = 'auto'), differences=1, lag=12),differences=1,lag=1))
```
...
svarma_model <- sVARMA(data, order=c(2,1,1), sorder=c(1,1,1), s=12)
preds <- sVARMApred(svarma_model, 0, h=length(rent_test))
plot(preds$pred[,1])
plot(preds$pred[,2])
```
